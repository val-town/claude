...
1. **Fetch the Val Town guide first.** Most integrations have a guide under `https://docs.val.town/guides/` (e.g. `slack/agent/`, `discord/bot/`, `stripe/`), but the slug isn't always just the service name — some guides are grouped (databases, browser automation) or live on a sub-page. Find the page from the docs site rather than guessing its URL, and fetch it before writing any integration code.
...
If a service the user wants isn't in this list, still check the Val Town docs at `https://docs.val.town/` — the catalog evolves.
