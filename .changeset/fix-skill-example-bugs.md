...
- **sqlite-storage**: corrected the database-scope section — `std/sqlite/global.ts` is the *organization-scoped* DB (not "per-user") and returns keyed-object rows like `main.ts`, not `any[][]`.
